...
public class HeapFile implements DbFile {
    ...
    public void writePage(Page page) throws IOException {
        ...
        HeapPage tempPage = (HeapPage) page;
        byte[] data = tempPage.getPageData();
        RandomAccessFile raf = new RandomAccessFile(f,"rw");
        raf.seek((long) tempPage.getId().pageNumber() * BufferPool.getPageSize());
        raf.write(data);
        raf.close();
    }
    ...
    public ArrayList<Page> insertTuple(TransactionId tid, Tuple t)
        ...
        // not necessary for lab1|lab2
        // use transactionId to get a heap page
        HeapPage hpage = null;
        for (int i = 0; i < this.numPages(); i++)
        {
            PageId pid = new HeapPageId(this.getId(), i);
            HeapPage temp = (HeapPage) Database.getBufferPool().getPage(tid, pid, Permissions.READ_WRITE);
        ...
        {
            hpage.insertTuple(t);
            return new ArrayList<Page> (List.of(hpage));
        }

        // if no empty pages found, create a new one
        HeapPageId newHeapPageId = new HeapPageId(this.getId(), this.numPages());
        HeapPage newHP = new HeapPage(newHeapPageId, HeapPage.createEmptyPageData());
        newHP.insertTuple(t);

        RandomAccessFile raf = new RandomAccessFile(this.f, "rw");
        int offset = BufferPool.getPageSize() * this.numPages();
        raf.seek(offset);
        byte[] newHPData = newHP.getPageData();
        raf.write(newHPData, 0, BufferPool.getPageSize());
        raf.close();

        return new ArrayList<Page> (List.of(newHP));
    }
    ...
    public ArrayList<Page> deleteTuple(TransactionId tid, Tuple t) throws DbException,
        ...
        hp.deleteTuple(t);
        return new ArrayList<Page> (List.of(hp));
